...
class MyBackoff(Backoff):
    def backoff(self, n):
        return self.base * random.randint(0, self.expo(n))
...
golden_ratio_conjugate = 2 / (1 + (5 ** 0.5))


def generate_numbers(mod):
    add = mod * golden_ratio_conjugate
    value = 0
    yield value
    while True:
        value = (value + add) % mod
        ...
        for backoff in backoff_types:
            ...
            with open("ts_" + backoff[1], "w") as ts_f:
                ...
                for t in range(0, 100):
                    ...
                    queue, stats = setup_sim(clients, backoff[0], ts_f, stats)
                    tm += run_sim(queue)
                data = backoff_data.setdefault(backoff[1], {})
                data.setdefault('work', {})[clients] = stats.calls / 100
                data.setdefault('time', {})[clients] = tm / 100
                print('')

    work_data = []
    time_data = []
    colors = [hsv_to_rgb((h, 1, 0.875)) for h in islice(generate_numbers(1.0), len(backoff_data))]
    for i, (backoff, data) in enumerate(backoff_data.items()):
        clients, work = zip(*sorted(data['work'].items()))
        plt.plot(clients, work, color=colors[i], label=backoff)
        plt.xlabel('Competing Clients')
        plt.ylabel('Work (calls)')
        plt.legend()
    plt.savefig('work.png', bbox_inches='tight')
    plt.clf()
    for i, (backoff, data) in enumerate(backoff_data.items()):
        clients, time = zip(*sorted(data['time'].items()))
        plt.plot(clients, time, color=colors[i], label=backoff)
        plt.xlabel('Competing Clients')
